<script lang="ts">
  ...
  const toggleEditModeAndSaveToDB = async () => { 
    if (isEditMode) {
...
{#snippet bookInfo()}
  ...
  {#if book.description}
    <h4 class="mt-m mb-xs semibold">Description</h4>
  ...
  {#if !book.finished_reading_on}
    <Button isSecondary={Boolean(book.started_reading_on)} onclick={updateReadingStatus}>
      {book.started_reading_on ? "I finished reading this book!" : "I sarted reading this book"}
    </Button>
  {/if}
  {#if book.genre}
...
{#snippet editFields()}
  <form>
    <input type="text" name="title" class="input input-title mt-m mb-xs" bind:value={title} />
    ...
    <textarea name="description" class="textarea mb-m" bind:value={description} placeholder={"Give a description."}></textarea>
    {#if !book.finished_reading_on}
      <Button isSecondary={Boolean(book.started_reading_on)} onclick={updateReadingStatus}>
        {book.started_reading_on ? "I finished reading this book!" : "I sarted reading this book"}
      </Button>
  ...
  <div class="book-container">
    <div class="book-info">
      ...
      <div class="buttons-container mt-m">
        <Button isSecondary={true} onclick={toggleEditModeAndSaveToDB}>{isEditMode ? "Save Changes": "Edit"}</Button>
