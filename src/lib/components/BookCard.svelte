<script lang="ts">
  ...
  // let {book}: {book: Book} = $props() // Instead of all the braces, define a bookcard prop interface
  let {book}: BookCardProps = $props()

  // Nested ternary operator
  let bookStatus = $derived(
    book.finished_reading_on ? "Finished" : book.started_reading_on ? "Currently reading" : "Not started"
  )

  // Non-nested ternary method
